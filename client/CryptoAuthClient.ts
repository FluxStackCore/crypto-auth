...
export class CryptoAuthClient {
  ...
  constructor(config: AuthConfig = {}) {
    ...
    if (this.config.storage === 'localStorage' && typeof localStorage !== 'undefined') {
      console.warn('[CryptoAuthClient] WARNING: Using localStorage to store private keys is insecure and vulnerable to XSS attacks. Consider using "memory" storage instead.')
  ...
  private signMessage(message: string, timestamp: number, nonce: string): string {
    ...
    // Ed25519 already performs internal hashing (SHA-512) per RFC 8032.
    // Signing a pre-hashed message is non-standard and incompatible with
